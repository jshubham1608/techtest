...
class SearchTrainViewController: UIViewController {
    ...
    @IBAction func addFavBtnClicked(_ sender: Any) {
        ...
        dropDown = DropDown()
        dropDown.anchorView = addFavBtn
        dropDown.direction = .bottom
        dropDown.bottomOffset = CGPoint(x: 0, y:(dropDown.anchorView?.plainView.bounds.height)!)
        dropDown.dataSource = stationsList.map {$0.stationDesc}
        dropDown.selectionAction = { (index: Int, item: String) in
            
            self.favBtn.setTitle(item, for: .normal)
            UserDefaults.standard.set(item, forKey: "favStation")
    ...
    @IBAction func favBtn(_ sender: Any) {
      ...
      let favStation =   UserDefaults.standard.object(forKey: "favStation") as? String
        
        
        if favStation != nil && favStation != "" {

            if sourceTxtField.isFirstResponder{
                sourceTxtField.text = favStation
            }else if destinationTextField.isFirstResponder{
                destinationTextField.text = favStation
            }
...
extension SearchTrainViewController:UITextFieldDelegate {
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        if let inputedText = textField.text {
            ...
            dropDown.dataSource = [desiredSearchText]
            dropDown.show()
